bl_info = {
    "name": "Blender Render Manager Sender",
    "author": "BRM Integration",
    "version": (1, 0, 0),
    "blender": (3, 0, 0),
    "location": "View3D > Sidebar > BRM",
    "description": "Send current scene render settings to Blender Render Manager",
    "category": "Render",
}
...
class BRMProperties(bpy.types.PropertyGroup):
    ...
    brm_path: StringProperty(
        ...
        subtype="FILE_PATH",
        default="",
        description="Path to Blender Render Manager executable or app.py",
    )


class BRM_OT_SendToManager(bpy.types.Operator):
    bl_idname = "brm.send_to_manager"
    bl_label = "Send to Blender Render Manager"
    bl_description = "Send current file/scene settings to BRM (opens BRM if needed)"
    ...
    def execute(self, context):
        ...
        try:
            ...
            if resp.get("ok"):
                self.report({"INFO"}, "Job enviado a Blender Render Manager.")
                return {"FINISHED"}
        except Exception:
            resp = {"ok": False}
...
class BRM_PT_Panel(bpy.types.Panel):
    bl_label = "Blender Render Manager"
