...
class ConvertThread(QThread):
    """Converts a PNG sequence to MP4 using FFmpeg (from PATH)."""
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        self._btn_convert = QPushButton("🎬  Convert to MP4")
        self._btn_convert.setToolTip(
            "Convert the sequence of PNGs from the selected job to an MP4 file.\n"
        ...
        self._btn_convert.clicked.connect(self._convert_to_mp4)
        self._fps_label = QLabel("FPS: —")
        self._fps_label.setStyleSheet(f"color: {C['teal']}; font-size: 9pt;")
        export_layout.addWidget(self._btn_convert)
        export_layout.addSpacing(12)
        export_layout.addWidget(self._fps_label)
        ...
        self.log_edit.verticalScrollBar().valueChanged.connect(self._on_log_scrolled)
    ...
    def _manage_profiles_dialog(self) -> None:
        ...
        def refresh_list(select_name: str | None = None) -> None:
            ...
            if select_name:
                ...
                for i in range(list_w.count()):
                    if list_w.item(i).text() == select_name:
        ...
        def flush_row_from_fields() -> bool:
            ...
            list_w.item(row).setText(edited[row].name)
            return True
    ...
    def _update_progress_ui(self, job: RenderJob):
        ...
        self._prog_vars["device"].setText(
            getattr(job, "_detected_device", "—")
        )
    ...
    def _retry_selected(self):
        ...
        for job in jobs:
            ...
            job._is_paused       = False
            if hasattr(job, "_detected_device"):
        ...
        def _play():
            try:
                ...
                if os.path.isfile(wav):
                    ...
                    _winsound.Beep(800,  200)
                    time.sleep(0.08)
                    _winsound.Beep(1000, 200)
                    time.sleep(0.08)
                    _winsound.Beep(1200, 400)
            except Exception:
                pass
    ...
    def _update_export_ui(self, job: RenderJob):
        """Enable/disable the Convert button and update the FPS label."""
    ...
    def _convert_to_mp4(self):
        ...
        output_file = os.path.join(job.output_path, f"{file_prefix}.mp4")
        ...
        self._btn_convert.setEnabled(False)
        self._fps_label.setText(f"FPS: {fps:g}  ⏳ convirtiendo…")
        self.status_bar.showMessage(
            f"Convirtiendo a MP4 a {fps:g} fps — {output_file}"
        )
